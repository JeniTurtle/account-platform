...
class RabbitMQ extends EventEmitter {
  ...
  async assertAllExchange() {
    ...
    Object.getOwnPropertyNames(this._exchanges).forEach(async index => {
      ...
      if (exchange.deadLetterExchange) {
        exchangeList.push(await this.assertExchange(exchange.deadLetterExchange, exchange.type, exchange.options));
      }
    });
  ...
  ack() {
    ...
    // @ts-ignore
    return this._channel.ack.apply(this._channel, arguments);
  }
  ...
  nack() {
    ...
    // @ts-ignore
    return this._channel.nack.apply(this._channel, arguments);
  }
  ...
  async createBinding(queue, exchange) {
    ...
    for (const index in queue.keys) {
      ...
      if (queue.enableDLX) {
        await this.bindQueue(queue.name, exchange.deadLetterExchange, key);
      }
    }
  }
  ...
  async createAllBinding() {
    ...
    return await Promise.all(
      Object.getOwnPropertyNames(this._queues).map(async index => {
        ...
        if (exchange) {
          // 如果队列启用死信交换机，则设置为绑定的交换机配置中的死信交换机
          if (queue.enableDLX) {
            queue.options.deadLetterExchange = exchange.deadLetterExchange;
          }
      ...
      await consumer(
        ...
        data => {
          ...
          callbackOptions.forEach(async option => {
            if (option.queue === queue.name && data.fields.routingKey === option.routingKey) {
